...
def _automation_settings_from_payload(payload: dict[str, Any], defaults: AutomationSettings | None = None) -> AutomationSettings:
    ...
    return AutomationSettings(
        ...
        codex_default_model=str(payload.get("codex_default_model", defaults.codex_default_model)),
...
def create_app(
    ...
    *,
    ...
    @app.get("/api/stories/{story_slug}/scenes/{scene_slug}/builder/interview-seed")
    def scene_builder_interview_seed(story_slug: str, scene_slug: str) -> dict[str, Any]:
        """Rebuild the interview seed for a scene imported from a candidate."""
        zet_app = _app(app.state.config_path)
        try:
            return _jsonable(zet_app.scene_builder_interview_seed(story_slug, scene_slug))
        except Exception as exc:
            raise HTTPException(status_code=400, detail=str(exc)) from exc
    ...
    def record_local_body_reference_analysis(
        ...
        try:
            ...
            return service.record_analysis(
                ...
                payload.get("input_hashes") if isinstance(payload.get("input_hashes"), dict) else {},
            )
        except Exception as exc:
            raise HTTPException(status_code=400, detail=str(exc)) from exc
