...
const promptEvolutionFixedSeedCount = document.querySelector("#prompt-evolution-fixed-seed-count");
...
const sceneRenameTitle = document.querySelector("#scene-rename-title");
const sceneRename = document.querySelector("#scene-rename");
...
function orderControls(label, index, count, onMove) {
  const cell = document.createElement("td");
  const previous = document.createElement("button");
  previous.type = "button";
  previous.textContent = "↑";
  previous.setAttribute("aria-label", `Move ${label} up`);
  previous.disabled = index === 0;
  previous.addEventListener("click", () => onMove(-1));
  const next = document.createElement("button");
  next.type = "button";
  next.textContent = "↓";
  next.setAttribute("aria-label", `Move ${label} down`);
  next.disabled = index === count - 1;
  next.addEventListener("click", () => onMove(1));
  cell.className = "order-controls";
  cell.append(previous, next);
  return cell;
}
...
function showFullscreenSceneDocument(document, scene) {
...
async function navigateFullscreenScene(offset) {
  ...
  try {
    ...
    showFullscreenSceneDocument(document, scene);
  } finally {
...
async function openStoryViewer() {
  ...
  try {
    ...
    if (!scenes.length) {
      showStoryMessage("This story has no scenes.");
      return;
    }
    const scene = scenes[0];
    const detail = await fetchJson(
      `/api/stories/${encodeURIComponent(state.selectedStorySlug)}/scenes/${encodeURIComponent(scene.slug)}`,
    );
    resetFullscreenNavigation();
    fullscreenSceneNavigation = { index: 0, scenes, storySlug: state.selectedStorySlug };
    showFullscreenSceneDocument(detail.document || {}, scene);
    updateFullscreenNavigation();
    resetFullscreenCrop();
...
function updateHeaderFitmentPreview() {
  // Show the locked Front-Left-3-4 Head-Image when this phase has one.
...
function isBaseImageAsset(asset) {
  return (
    ["Body-Reference", "Head-Image", "Character-Assembly"].includes(asset?.pipeline || "") &&
...
function updateActionButtons(detail) {
  ...
  for (const button of actionButtons) {
    ...
    if (action === "keep-locked") {
      enabled = enabled && lockedExists && (asset.asset_state !== "LOCKED" || asset.pipeline_stage !== "LOCKED");
...
async function reorderStory(index, offset) {
  const target = index + offset;
  if (target < 0 || target >= state.stories.length) return;
  const slugs = state.stories.map((story) => story.slug);
  [slugs[index], slugs[target]] = [slugs[target], slugs[index]];
  try {
    const payload = await fetchJson("/api/stories/order", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
    ...
    showStoryMessage(payload.message || "Story order saved.");
  } catch (error) {
    showStoryMessage(error.message, "error");
  }
}
...
function appendStorySettingFields(container, value, path = [], readOnlyFields = []) {
  ...
  if (value && typeof value === "object") {
    const keys = Object.keys(value).filter((key) => !(path.length === 0 && key === "scene_index"));
    if (!keys.length) {
      container.append(renderStorySettingField(path, value));
      return;
    }
...
async function renameStory() {
  if (!state.selectedStorySlug) return;
  const title = storyRenameTitle.value.trim();
  if (!title) {
    showStoryMessage("Story title is required.", "error");
    return;
  }
  ...
  try {
    ...
    state.stories = payload.stories || state.stories;
    state.storyDetail = payload.document || state.storyDetail;
    updateStoryGitWarning(payload.has_story_changes);
...
function clearSceneEditor() {
  ...
  sceneRename.disabled = true;
  sceneMove.disabled = true;
...
async function reorderScene(index, offset) {
  const target = index + offset;
  if (target < 0 || target >= state.scenes.length || !state.selectedStorySlug) return;
  const slugs = state.scenes.map((scene) => scene.slug);
  [slugs[index], slugs[target]] = [slugs[target], slugs[index]];
  try {
    const payload = await fetchJson(`/api/stories/${encodeURIComponent(state.selectedStorySlug)}/scenes/order`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ slugs }),
    });
...
async function renameScene() {
  ...
  try {
    const payload = await fetchJson(
      `/api/stories/${encodeURIComponent(state.selectedStorySlug)}/scenes/${encodeURIComponent(state.selectedSceneSlug)}`,
      {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ title }),
    ...
    sceneRenameTitle.value = state.sceneDetail?.scene?.title || title;
...
async function moveScene() {
  ...
  const targetStorySlug = sceneMoveStory.value;
  if (!confirm(`Move ${sceneSlug} to ${targetStorySlug}?`)) return;
  sceneMove.disabled = true;
  try {
    const payload = await fetchJson(
      `/api/stories/${encodeURIComponent(sourceStorySlug)}/scenes/${encodeURIComponent(sceneSlug)}/move`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ target_story_slug: targetStorySlug }),
      },
    );
...
function builderTargetParentId(subscene) {
  if (!subscene || subscene.kind !== "element") return "main";
  const anchor = (state.sceneBuilder?.scene_elements || []).find((item) => item.id === subscene.anchor_element_id);
  return anchor?.subscene_id || "main";
}

function builderTargetDepth(targetId, seen = new Set()) {
  if (!targetId || targetId === "main") return 0;
  const target = (state.sceneBuilder?.subscenes || []).find((item) => item.id === targetId);
  if (!target || target.kind !== "element") return 0;
  if (seen.has(targetId)) return 99;
  seen.add(targetId);
  return builderTargetDepth(builderTargetParentId(target), seen) + 1;
}

function builderTargetChildren(targetId) {
  return (state.sceneBuilder?.subscenes || []).filter((item) => builderTargetParentId(item) === targetId);
}

function builderTargetPath(targetId) {
  const path = [];
  const seen = new Set();
  let currentId = targetId;
  while (currentId && currentId !== "main" && !seen.has(currentId)) {
    seen.add(currentId);
    const current = (state.sceneBuilder?.subscenes || []).find((item) => item.id === currentId);
    if (!current) break;
    path.unshift(current);
    currentId = builderTargetParentId(current);
  }
  return path;
}

function builderCanMoveElementToTarget(element, targetId) {
  const owned = builderSubsceneForAnchor(element?.id);
  if (!owned) return true;
  if (targetId === owned.id) return false;
  const descendants = new Set();
  const visit = (parentId) => {
    for (const child of builderTargetChildren(parentId)) {
      if (child.kind !== "element" || descendants.has(child.id)) continue;
  ...
  const deepest = Math.max(builderTargetDepth(owned.id), ...[...descendants].map((id) => builderTargetDepth(id)));
  const subtreeHeight = deepest - builderTargetDepth(owned.id) + 1;
  return builderTargetDepth(targetId) + subtreeHeight <= 3;
}
...
function builderRemoveSelectedElement() {
  ...
  if (builderSubsceneForAnchor(element.id)) {
    showSceneBuilderMessage("Disable does not remove an element sub-scene. The anchor cannot be deleted while it owns a sub-scene.", "error");
    return;
  }
...
function builderRenderPlacementEditor() {
  ...
  if (!placement) {
    return `<section class="scene-builder-editor-section"><h5>${escapeHtml(placementTitle)}</h5><p>Select or add a scene element.</p></section>`;
  }
  ...
  return `
    <section class="scene-builder-editor-section">
      ...
      <div class="scene-builder-fields">
        ...
        <label class="full">${builderCaption("Placement instructions — included in prompt", "placements[].placement_notes")}<textarea data-builder-placement-field="placement_notes"${placementDisabled}>${escapeHtml(placement.placement_notes || "")}</textarea></label>
      </div>
    </section>
  `;
}
    ...
    <div class="scene-builder-card scene-builder-element-workspace">
      <div class="review-header">
        <div>
          ...
          <h4>${escapeHtml(element.display_name || element.id || "Element")}</h4>
        </div>
...
function builderRenderOverrides() {
  return `
    <details class="scene-builder-card scene-builder-advanced">
...
function builderRenderTechnicalDetails() {
  const scene = state.sceneBuilder.scene || {};
  return `
    <details class="scene-builder-technical-details">
      <summary>Technical Details</summary>
      <dl>
        <dt>Scene JSON</dt><dd><code>${escapeHtml(scene.slug || "")}.scene.json</code></dd>
        <dt>Story settings</dt><dd><code>${escapeHtml(scene.story_settings_path || "Not set")}</code></dd>
        <dt>Scene image</dt><dd><code>${escapeHtml(scene.associated_png_path || "Not set")}</code></dd>
      </dl>
    </details>
  `;
}

function builderRenderMoreMenu() {
...
function renderSceneCandidateStoryOptions(defaultStory = "") {
  ...
  if (!sceneCandidateStory) return;
  const selected = state.selectedSceneCandidateStorySlug || state.selectedStorySlug || defaultStory;
  ...
  state.selectedSceneCandidateStorySlug = sceneCandidateStory.value || null;
...
function renderSceneCandidateDetail() {
  ...
  const fields = Object.entries(candidate.fields || {}).filter(([label]) => label !== "Candidate ID").map(([label, value]) => {
  ...
  const rendered = candidate.import_state === "rendered";
  const passed = candidate.import_state === "passed";
  const image = rendered
    ? `<img class="scene-candidate-image" src="/api/scene-candidates/${encodeURIComponent(candidate.source_key)}/${encodeURIComponent(candidate.candidate_id)}/image" alt="Rendered scene ${escapeHtml(candidate.title)}">`
    : "";
  const actions = passed
    ? `<button id="scene-candidate-activate" class="primary-action" type="button">Activate</button>`
    : `<button id="scene-candidate-import" class="primary-action" type="button">${actionLabel}</button>${rendered ? "" : '<button id="scene-candidate-pass" type="button">Pass</button>'}`;
  const storySelector = !passed && (!imported || candidate.import_state === "source_changed")
    ? '<label class="scene-candidate-story-label">Import into:<select id="scene-candidate-story"></select></label>'
    : "";
  sceneCandidateDetail.innerHTML = `
    ...
    <div class="button-row compact">${storySelector}${actions}</div>
    ${image}
    <section><h3>Candidate ID</h3><pre>${escapeHtml(candidate.candidate_id)}</pre></section>
    ${fields}
  `;
  renderSceneCandidateStoryOptions();
  document.querySelector("#scene-candidate-import")?.addEventListener("click", importSelectedSceneCandidate);
  document.querySelector("#scene-candidate-pass")?.addEventListener("click", () => setSelectedSceneCandidatePassed(true));
  document.querySelector("#scene-candidate-activate")?.addEventListener("click", () => setSelectedSceneCandidatePassed(false));
}

function renderSceneCandidates() {
  const filter = sceneCandidateFilter.value || "in_progress";
  const candidates = state.sceneCandidates.filter((item) => filter === "in_progress"
    ? !["rendered", "passed"].includes(item.import_state)
    : item.import_state === filter);
  if (!candidates.some((item) => item.candidate_id === state.selectedSceneCandidateId)) {
    state.selectedSceneCandidateId = candidates[0]?.candidate_id || null;
  }
...
async function setSelectedSceneCandidatePassed(passed) {
  const candidate = selectedSceneCandidate();
  if (!candidate) return;
  showSceneCandidateMessage(passed ? "Passing candidate..." : "Activating candidate...");
  try {
    await fetchJson(`/api/scene-candidates/${encodeURIComponent(candidate.source_key)}/${encodeURIComponent(candidate.candidate_id)}/passed`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ passed }),
    });
    await loadSceneCandidates();
    showSceneCandidateMessage(passed ? "Candidate passed." : "Candidate activated.", "success");
  } catch (error) {
...
function builderRenderTargetControls() {
  ...
  const renderChildren = (parentId, seen = new Set()) => builderTargetChildren(parentId).map((item) => {
    ...
    const depth = item.kind === "element" ? builderTargetDepth(item.id) : 0;
    return targetButton(item, depth) + renderChildren(item.id, nextSeen);
  }).join("");
  const tabs = targetButton({ id: "main", name: "Full Scene", enabled: true }, 0) + renderChildren("main");
  const breadcrumbs = builderTargetPath(state.activeBuilderRenderTarget).map((item) => escapeHtml(item.name || item.id)).join(" › ");
  const activeSubscene = builderActiveSubscene();
  ...
  const elementToggle = activeSubscene?.kind === "element"
    ? activeSubscene.enabled
...
function builderRenderSubsceneSettings() {
  ...
  if (subscene.kind === "element") {
    const targetSetup = subscene.setup || {};
    const canvas = targetSetup.canvas || {};
    ...
    return `<div class="scene-builder-card">
      ...
      <div class="scene-builder-fields">
        ...
        <label class="full">Composition notes<textarea data-builder-element-subscene-field="composition.composition_notes">${escapeHtml(composition.composition_notes || "")}</textarea></label>
        <label class="full">Independent location<textarea data-builder-element-subscene-field="environment.location">${escapeHtml(targetEnvironment.location || "")}</textarea></label>
        ${environmentField("lighting", "Lighting")}
        ${environmentField("mood", "Mood")}
        ${environmentField("weather_or_atmosphere", "Weather / atmosphere")}
        <label class="full">General foreground notes<textarea data-builder-element-subscene-field="environment.general_foreground_notes">${escapeHtml(targetEnvironment.general_foreground_notes || "")}</textarea></label>
        <label class="full">General background notes<textarea data-builder-element-subscene-field="environment.general_background_notes">${escapeHtml(targetEnvironment.general_background_notes || "")}</textarea></label>
      </div>
    </div>`;
  }
  return `<div class="scene-builder-card">
    ...
    <div class="scene-builder-grid">
      ...
      <section id="builder-panel-scene" class="scene-builder-section scene-builder-foundation" data-builder-section-panel="scene" aria-label="Scene foundation">
        ${activeSubscene ? `<div class="scene-builder-card scene-builder-story-beat">
          ...
          <h3>Story Beat</h3>
          <div class="scene-builder-fields">
            ${builderField("scene.name", "Scene title", "", true)}
      ...
      <section id="builder-panel-environment" class="scene-builder-section scene-builder-environment" data-builder-section-panel="environment" aria-label="Environment">
        ${activeSubscene ? `<div class="scene-builder-card"><h4>${activeSubscene.kind === "element" ? "Target environment" : "Inherited environment"}</h4><p>${activeSubscene.kind === "element" ? "Lighting, mood, and weather can inherit from the immediate parent, be overridden, or be omitted under Scene." : "Edit universal environment fields in Full Scene. Target-specific notes are available under Scene."}</p></div>` : builderRenderEnvironment()}
      </section>
...
async function enableSelectedElementSubscene(elementId = "") {
  ...
  try {
    ...
    state.sceneBuilder = payload.document?.data || state.sceneBuilder;
    state.sceneBuilderRenderTargets = payload.document?.render_targets || [];
    state.activeBuilderRenderTarget = payload.render_target_id || builderSubsceneForAnchor(element.id)?.id || "main";
    state.selectedBuilderElementId = state.sceneBuilder.scene_elements?.find((item) => item.subscene_id === state.activeBuilderRenderTarget)?.id || element.id;
    state.selectedBuilderPlacementId = builderPlacementForElement(state.selectedBuilderElementId)?.id || null;
    updateStoryGitWarning(payload.has_story_changes);
    renderSceneBuilder();
    state.savedBaselines.sceneBuilder = sceneBuilderSnapshot();
...
async function disableSceneSubscene(targetId) {
  ...
  try {
    ...
    state.sceneBuilder = payload.document?.data || state.sceneBuilder;
    state.sceneBuilderRenderTargets = payload.document?.render_targets || [];
    state.activeBuilderRenderTarget = builderTargetParentId((state.sceneBuilder.subscenes || []).find((item) => item.id === targetId)) || "main";
    state.selectedBuilderElementId = state.sceneBuilder.scene_elements?.find((item) => !item.subscene_id || item.subscene_id === state.activeBuilderRenderTarget)?.id || null;
    state.selectedBuilderPlacementId = builderPlacementForElement(state.selectedBuilderElementId)?.id || null;
    updateStoryGitWarning(payload.has_story_changes);
    renderSceneBuilder();
    state.savedBaselines.sceneBuilder = sceneBuilderSnapshot();
...
sceneBuilderPanel.addEventListener("click", (event) => {
  ...
  if (target.dataset.builderSelectElement) {
    ...
    if (action === "disable-subscene") {
      ...
      if (targetId === "background") setBackgroundSubsceneEnabled(false);
      else disableSceneSubscene(targetId);
    }
    ...
    if (action === "open-page" && target.dataset.builderPage) activatePage(target.dataset.builderPage);
  }
});
...
function selectedPromptReviewScene() {
  ...
  return manifest.story_slug && manifest.scene_slug
    ? { storySlug: manifest.story_slug, sceneSlug: manifest.scene_slug, renderTargetId: manifest.render_target_id || "main" }
    : null;
}
...
async function analyzePromptReview() {
  ...
  try {
    const analysis = await fetchJson(`/api/stories/${encodeURIComponent(scene.storySlug)}/scenes/${encodeURIComponent(scene.sceneSlug)}/prompt-analysis?render_target_id=${encodeURIComponent(scene.renderTargetId)}`, { method: "POST" });
...
function renderPromptEvolutionDetail(run) {
  ...
  const imageGrid = (items) => `<div class="prompt-evolution-gallery">${(items || []).map((item) => `<article class="prompt-evolution-candidate"><img class="fullscreen-image-trigger" src="${promptEvolutionFileUrl(item.file)}" alt="Seed ${escapeHtml(item.seed)}"><strong>${escapeHtml(item.seed_role || "seed")} ${escapeHtml(item.seed)}</strong></article>`).join("")}</div>`;
  const finalReview = run.status === "AWAITING_FINAL_REVIEW" ? `<section class="prompt-evolution-summary"><h2>Choose the best prompt version</h2><p>Choices are randomized. Prompt history and diffs remain available below.</p>${(run.prompt_versions || []).map((version, index) => `<article class="prompt-evolution-batch"><h3>Choice ${String.fromCharCode(65 + index)}</h3>${imageGrid(version.fixed_renders)}<details><summary>Fresh-seed evidence</summary>${imageGrid(version.fresh_renders)}</details><button type="button" data-prompt-evolution-final-version="${escapeHtml(version.prompt_version_id)}">Choose this prompt version</button></article>`).join("")}<label>Optional selection reason<textarea data-prompt-evolution-selection-reason></textarea></label></section>` : "";
  const promptDetails = (batch, previous) => {
    const positive = batch.positive_prompt || batch.positive_core || "";
    const negative = batch.negative_prompt || batch.negative_core || "";
    const previousPositive = previous?.positive_prompt || previous?.positive_core || "";
    const previousNegative = previous?.negative_prompt || previous?.negative_core || "";
    const diff = previous ? `<h4>Diff from previous version</h4><div class="prompt-evolution-prompts"><strong>Positive</strong> ${promptEvolutionPromptDiff(positive, previousPositive)}\n\n<strong>Negative</strong> ${promptEvolutionPromptDiff(negative, previousNegative)}</div>` : "";
  ...
  const activityLog = `<section class="prompt-evolution-log" aria-label="Run activity log"><h2>Run activity</h2><ol>${(run.activity_log || []).map((event) => `<li class="${escapeHtml(event.level || "info")}"><time>${escapeHtml(String(event.at || "").replace("T", " "))}</time><span>${escapeHtml(event.message || "")}</span></li>`).join("") || "<li>No activity recorded for this run.</li>"}</ol></section>`;
...
const PROMPT_EVOLUTION_ACTIVE_STATUSES = new Set(["BOOTSTRAPPING", "RENDERING", "OBSERVING", "SYNTHESIZING", "DIAGNOSING", "EDITING", "DIRECTED_REFINING"]);
...
async function loadManifestTasks(preferredAssetId = null) {
  ...
  const [headImagePayload, assemblyPayload] = await Promise.all([
  ...
  state.manifestTasks = [...(headImagePayload.tasks || []), ...(assemblyPayload.tasks || [])];
...
async function uploadHeadshotReference(file = headshotUpload.files?.[0]) {
  ...
  try {
    const uploadRoute = "/api/head-image-sources";
...
sceneRename.addEventListener("click", renameScene);
sceneMove.addEventListener("click", moveScene);
...
promptEvolutionDetail.addEventListener("click", async (event) => {
  ...
  const finalVersion = event.target.closest("[data-prompt-evolution-final-version]");
  ...
  try {
    ...
    if (finalVersion) {
      ...
      renderPromptEvolutionDetail(await fetchJson(`/api/prompt-evolution/runs/${encodeURIComponent(run.run_id)}/final-selection/${encodeURIComponent(finalVersion.dataset.promptEvolutionFinalVersion)}`, {
