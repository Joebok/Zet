...
def run(asset, context) -> WorkerResult:
    ...
    if len(references) > 1:
        return WorkerResult(False, "Head-Image accepts at most one source image.", advance_stage=False, error_code="INVALID_REFERENCE")
    if references:
    ...
    return WorkerResult(True, f"Resolved optional Head-Image source for Asset {asset.asset_id}.", advance_stage=True, reference_files=references)
