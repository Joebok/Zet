...
* New invariants introduced: `ImageCatalog.json` is a schema-v3 complete manifest; catalog and reference-set data live in one atomic JSON record per ID, while collections/keywords live in `Organization.json`. Library-owned image paths are library-relative with `image_path_kind: library`; external paths remain absolute with `image_path_kind: external`. Runtime construction and ASGI startup reject legacy or incomplete catalog storage with the explicit migration command.
* APIs/contracts changed: `ImageCatalogRepository` now strictly reads/writes the record layout and performs differential per-record/reference/organization writes and backups. The explicit command is `.venv\Scripts\python.exe -m zet.scripts.migrate_image_catalog --config <path-to-config.toml>` (add `--dry-run` for write-free preflight); its JSON report exposes status/counts/hash verification, backup path, and staging path. Catalog provenance sources now identify the applicable catalog or reference-set record rather than the manifest. The exact layout and operating procedure are recorded in `Docs/ImageCatalogMigration.md` for WP09.
* New tests/fixtures available to later WPs: `tests/test_wp08_catalog_migration.py` covers dry-run immutability, ID/source-key/reference/metadata/raw-override/provenance retention, library-relative paths, image hashes, legacy backup, repeat-run no-op, durable interruption/resume, incomplete-format rejection, and affected-record-only edit/backup behavior. The WP01 reliability fixture now emits schema-v3 catalogs; legacy auxiliary tests invoke the explicit migrator.
* Assumptions later WPs may rely on: WP08 acceptance is PASS. The focused WP08/catalog/web/WP01/WP04 suite passed 36 tests, the full Python suite passed 269 tests plus 43 subtests, all 3 Image Inventory browser tests passed, and the isolated WP01 benchmark passed at 1x/10x/100x cold and warm scales. No live library was migrated, and no unresolved validation blocker remains. WP09 may index `Records/*.json`, hashed `ReferenceSets/*.json` (using the stored `reference_set_id`), and `Organization.json`; it must not treat `.migration-v3` or `_backup` as authored current records.
* Deviations from master plan: The module-level ASGI object defers validation until the actual lifespan startup so importing the web module does not read and reject the unmigrated live library during isolated test discovery; direct `ZetApp.from_config` and ordinary `create_app` construction remain strict. The resumable staging directory is retained after success as migration evidence; runtime ignores it once a complete v3 manifest is active.
...
* New invariants introduced: Each canonical authored-library path maps to a SHA-256-keyed SQLite database under machine-local `%LOCALAPPDATA%/Zet/Indexes` by default; configured index roots inside the library, Dropbox, or a Git worktree are rejected. Rebuilds publish one complete generation transactionally, queries retain the prior complete generation until activation succeeds, and source paths are stored relative to either `library/` or `queue/` with SHA-256 fingerprints and explicit valid/error parse status.
* APIs/contracts changed: `LibraryIndexRepository` exposes building/ready status, deterministic cursor pagination, scope/status/name queries, explicit corruption recovery guidance, and generation publication. `LibraryIndexService.rebuild()` indexes stories, ordered scenes, render targets, WP08 catalog records/reference relationships, active queue work, and completed/archive job summaries. The explicit rebuild command is `.venv\Scripts\python.exe -m zet.scripts.rebuild_library_index --config <path-to-config.toml>`; dashboard list routes remain on their existing readers.
* New tests/fixtures available to later WPs: `tests/test_wp09_library_index.py` covers external database placement, authored-file hash preservation across delete/rebuild, building state, stable filtered pagination, transactional interruption, malformed scene/catalog records with valid-record retention, corruption guidance/recovery, and an unchanged-source second generation. `Scripts/Benchmark_WP09.py` measures isolated cold/warm rebuild and paginated-query performance at WP01 1x/10x/100x scales.
* Assumptions later WPs may rely on: WP09 acceptance is PASS. The focused WP08/WP09 suite passed 11 tests, the full Python discovery suite passed 251 tests, compile validation passed, and the WP09 benchmark indexed the expected entities with zero errors through 800 scenes/1,600 targets. Ruff validation was unavailable because Ruff is not installed; no dependency was installed and this is not an acceptance blocker. WP10 may use `sources` fingerprints and parse status for reconciliation, but must add incremental refresh/invalidation rather than altering WP09's explicit full-generation rebuild contract; WP11 may adopt the query APIs but no dashboard list currently consumes them.
* Deviations from master plan: Initial rebuild is explicit rather than a background startup task, as allowed by WP09. Historical summaries include ready Answer records and recursive archive records; active work includes non-submitted Ask and Running records. No broader dashboard or recurring reconciliation changes were made.
...
* New invariants introduced: One web-lifespan reconciler runs an immediate scan and then waits 60 seconds after each completed scan. Reconciliation publishes complete generations only when indexed source fingerprints change, reuses persisted parsed payloads for unchanged JSON, excludes backup/media/temporary/authored-archive contents, and serializes background and synchronous refreshes. Successful story, scene, catalog, scene-render, prompt-analysis, and harvest writes reconcile before returning and invalidate affected summary/compilation caches.
* APIs/contracts changed: `LibraryIndexService.reconcile()`, `record_compilation()`, and `compilation_is_current()` add incremental refresh and actual-file dependency validation; `LibraryIndexReconciler` owns the single restartable loop. `LibraryIndexRepository.status()` now exposes the persisted reconciliation cursor, last-completed UTC time, generation, changed sources, and errors. Its additive SQLite tables cache parsed sources and compiled scene results keyed by dependency fingerprint plus `scene_render_v4`, with edges covering scene JSON, story settings, prompt templates, resolved source records, reference-set records, and referenced images.
* New tests/fixtures available to later WPs: `tests/test_wp10_library_index_reconciliation.py` covers external create/edit/delete/rename, one-file-only reparsing, excluded roots/content, unrelated-edit cache retention, scene/settings/template/catalog dependency edges, dependency invalidation, actual dependency revalidation, synchronous app-write refresh, controlled-clock scheduling/status, and restart repair after a source-write/index-update crash. `Scripts/Benchmark_WP10.py` measures unchanged and one-changed-source reconciliation at WP01 1x/10x/100x scales.
* Assumptions later WPs may rely on: WP10 acceptance is PASS. The final WP09/WP10 focused suite passed 13 tests, the affected WP08-WP10/render/catalog suite passed 43 tests, the full Python discovery suite passed 257 tests, and compile validation passed. The final WP10 benchmark reused 1,204/1,204 sources for an unchanged 100x library in 1.912s and reparsed exactly 1/1,204 after one edit in 1.900s; the WP09 rebuild/query benchmark also passed through 800 scenes/1,600 targets. No unresolved validation blocker remains. Consequential scene staging and promotion continue to compile from actual current dependencies; a cached-fresh flag is never used as authorization. WP11 may consume the active generation and reconciliation status without adding a request-time full-scan fallback or another scheduler.
* Deviations from master plan: WP09's recursive queue `Archive` history remains indexed because it is an explicit historical-summary contract; the archive exclusion applies to authored archive/backup trees and non-manifest contents. Incremental generations reuse parsed JSON but still enumerate bounded relevant roots and reconstruct relational rows, avoiding a second incremental data model.
...
* New invariants introduced: Inventory, scene-candidate, image-review, production-count, and harvested-history reads use the active SQLite snapshot; public list pages default to 50 items and reject limits above 200; cursors bind to the generation plus index revision and fail as stale after publication or history backfill; scoped and project production counts execute in one SQLite transaction; scene-review freshness is calculated only for the returned page; ordinary reconciliation carries indexed history forward without traversing harvested archives.
* APIs/contracts changed: `IndexPage` now carries `items`, `total`, `next_cursor`, `generation`, and `freshness` (including bounded index errors). `GET /api/image-catalog`, `GET /api/scene-candidates`, `GET /api/render-review/tasks`, and `GET /api/ai-controls/recent-harvests` use that response shape exclusively with `cursor`/`limit`; `GET /api/production-work-summary` returns indexed counts plus generation/freshness metadata. `POST /api/library-index/history-backfill?batch_size=` is the explicit resumable archive import. `LibraryIndexService.rebuild(backfill_history=True)` remains an explicit full-rebuild/backfill operation, while automatic reconciliation does not scan archives.
* New tests/fixtures available to later WPs: `tests/test_wp11_indexed_dashboard.py` covers page limits/order/totals, stale generation and backfill cursors, indexed-only API reads, one-snapshot count parity, explicit resumable temporary-archive backfill, zero archive access on history reads, and fixed-page bounded work. `Scripts/Benchmark_WP11.py` compares 500 and 5,000 unrelated indexed items and asserts a 50-item page with zero scene loads and render compiles. Existing browser fixtures now use the indexed scene-candidate response.
* Assumptions later WPs may rely on: WP11 acceptance is PASS. The focused WP04/WP07/WP09/WP10/WP11 suites passed 46 tests, the full Python discovery suite passed 262 tests, JavaScript syntax validation passed, and the six affected dashboard browser tests passed. The WP11 benchmark returned 50 items with zero scene loads/compiles at both 1x and 10x; the WP10 benchmark still passed through 100x with unchanged reconciliation reusing 1,204/1,204 sources and one-edit reconciliation parsing 1/1,204. A full 33-test dashboard run had one repeatable pre-existing WP03 ordered-next-navigation failure while the other 32 tests passed; the affected WP11 browser subset passed 6/6, so no WP11 acceptance validation remains unresolved.
* Deviations from master plan: WP10 exposed indexed scene/catalog/work/history primitives but no rows rich enough to serve the existing dashboard contracts, so WP11 added generation-scoped serialized list rows populated during reconciliation. Incomplete legacy test fixtures can make an individual dashboard provider fail; that provider contributes a bounded index error and an empty affected list instead of aborting queue reconciliation. No live history archive was backfilled.
...
* New invariants introduced: ModelUpdater-managed Ollama aliases own `num_ctx` and `num_predict`; Zet requests retain task-owned temperature and response schemas. Successful direct and queued inference requires evidence containing the requested/effective alias, alias digest, and managed context/output settings. Unsupported AssetWorkflow stages and exhausted Prompt Evolution bootstrap retries fail explicitly before placeholder fallback inference.
* APIs/contracts changed: `[AIModels]` replaces `PromptEvolutionAnalysis` with `PromptEvolutionVision` and `PromptEvolutionText`; new Prompt Evolution runs snapshot both assignments while existing runs retain their recorded shared `analysis_model`. `OllamaModelService.generate_json_with_evidence()` returns parsed output plus runtime evidence, queue answer manifests add `ollama_runtime`, and the Ollama model-list response exposes all models plus `vision_models` capability metadata. Run `python3 -m zet.scripts.migrate_wp13_config --config <config.toml>` to copy a legacy shared assignment into both roles with a backup; runtime loading rejects the legacy key with this instruction.
* New tests/fixtures available to later WPs: `tests/test_wp13_runtime_roles.py` covers every selectable role, temporary-config migration/idempotence, text repair, vision directed refinement, and explicit fallback failure. Direct Scene Builder/Image Quality tests and queued worker tests provide managed runtime-evidence fixtures; Prompt Evolution and pipeline-control tests cover split-role routing and round trips.
* Assumptions later WPs may rely on: WP13 acceptance is PASS: the focused suite passed 33 tests plus 2 subtests, the Prompt Evolution browser test passed, and JavaScript syntax validation passed. No benchmark is defined for WP13. The full Zet suite remains non-green only for the two pre-existing WP12-documented `ZetApp.__new__` fixture failures (292 passed, 41 subtests passed, 2 failed). ModelUpdater runtime-contract test collection was unavailable because its environment lacks `jsonschema`; no dependency was installed. Changes were not staged because all requested validation was not green.
* Deviations from master plan: Existing Prompt Evolution run files keep their recorded `analysis_model` as a compatibility snapshot so in-progress/authored runs do not change assignments; only new runtime configuration requires the split keys. The tracked Zet config was updated by copying its former shared assignment to both roles, with no alias activation or reassignment. ModelUpdater's working tree already contained extensive unrelated user changes and was inspected but not modified.
