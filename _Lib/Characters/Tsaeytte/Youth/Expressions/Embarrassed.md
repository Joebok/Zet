...
Character: [Character Name], [Phase].
Canonical Art Style: [Canonical art style].
Expression label: Embarrassed.
Expression intensity: clear and readable, but not caricatured.
Identity Key: {{IDENTITY_KEY_LABEL}}.
Expression definition: _Lib\Characters\Tsaeytte\Youth\Expressions\Embarrassed.md.
...
[Expression Label] - [short expression summary].

[Describe the emotional target in plain language.]

Required facial expression:
- [required facial expression detail]
- [required facial expression detail]
...
- [small supporting posture/tension allowances]
- do not introduce a dramatic head tilt, theatrical hand gesture, or narrative action

Do not make the expression look like:
- [forbidden emotional drift]
- [forbidden expression category]
- generic neutral expression

The expression should be readable as the same character with a new expression only.
...
- Same character identity, age, species, face structure, eye color, ears, hair silhouette, and costume cues as the Identity Key.
...
- Output becomes a scene, sheet, collage, diagram, label page, or multi-view image.
...
[Expression Label]-expression-specific avoidance:

- [expression-specific forbidden result]
- [expression-specific forbidden result]
