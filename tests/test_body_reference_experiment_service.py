...
from zet.services.body_reference_experiment_service import (
    BodyReferenceExperimentError,
    BodyReferenceExperimentService,
...
def make_service(tmp_path: Path) -> BodyReferenceExperimentService:
    ...
    return BodyReferenceExperimentService(app, project)
...
def test_preview_has_eight_views_and_seventy_two_candidates(tmp_path):
    ...
    assert plan["candidate_count"] == 72


def test_codex_jobs_report_pending_running_done_and_failed(tmp_path):
    service = make_service(tmp_path)
    root = service.experiments_root / "Test" / "Adult" / "20260922_120000_000001"
...
def test_workflow_reference_binding_is_only_present_for_conditioned_method():
    ...
    conditioned = BodyReferenceExperimentService.compile_qwen_workflow(
...
def test_disposition_requires_two_independent_reviews():
    assert BodyReferenceExperimentService.disposition({}, {}) == "pending"
    assert BodyReferenceExperimentService.disposition({"pass": True}, {"pass": True}) == "joint_pass"
    assert BodyReferenceExperimentService.disposition({"pass": False}, {"pass": False}) == "joint_fail"
    assert BodyReferenceExperimentService.disposition({"pass": True}, {"pass": False}) == "human_triage"
    assert BodyReferenceExperimentService.disposition({"pass": True}, {"pass": True, "uncertain": True}) == "human_triage"
...
def test_front_anchor_requires_completed_reviewed_front_candidate(tmp_path, monkeypatch):
    ...
    with pytest.raises(BodyReferenceExperimentError):
...
def test_luna_schema_has_explicit_view_aware_criteria():
    schema = BodyReferenceExperimentService.analysis_schema()
...
def test_experiment_harvest_uses_its_own_slot_when_proxy_omits_target_directory(tmp_path):
    service = make_service(tmp_path)
    run_id = "20260922_000000_000001"
    root = service.experiments_root / "Tsaeytte" / "Adult" / run_id
    root.mkdir(parents=True)
    ...
    (answer_dir / "ask_manifest.json").write_text(json.dumps({
        "ask_id": ask_id, "body_reference_run_id": run_id, "candidate_id": "c001",
        "task_type": "body_reference_analysis", "expected_output": "response.json",
    ...
    service._harvest_experiment_answer(run_id, "c001", ask_id, target)
    ...
    service._harvest_experiment_answer(run_id, "c001", render_id, render_target)
    assert render_target.read_bytes() == b"image"


def test_analysis_prompt_uses_saved_facts_and_prioritizes_proportions():
    prompt = BodyReferenceExperimentService.analysis_prompt(
...
def test_existing_run_refreshes_analysis_when_template_changes(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("zet.services.body_reference_experiment_service.compile_body_reference_job", compile_analysis)
...
def test_luna_environment_drops_desktop_context(monkeypatch):
    ...
    env = BodyReferenceExperimentService._luna_environment()
...
def test_runner_batches_images_before_reviews_for_each_view(tmp_path, monkeypatch):
    ...
    run = service.create_run({
        ...
        "front_count": 2, "other_count": 1, "seeds": list(range(16)),
    })
    root = Path(run["root"])
